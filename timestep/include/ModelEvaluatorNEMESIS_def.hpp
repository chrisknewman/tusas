...
//                  ));
  ...
  //meshx = new double[xlen];//need to alloc this
  //std::copy((mesh_->x).begin(),(mesh_->x).end(),meshx);
  double *xxx = (mesh_->x).data();
#pragma omp target enter data map(alloc:xxx[0:xlen])
#endif
#endif
//   int xlen = (mesh_->x).size();
//   testptr = (mesh_->x).data();
//   double *xxx = testptr;
// #pragma omp target enter data map(alloc:xxx[0:xlen])

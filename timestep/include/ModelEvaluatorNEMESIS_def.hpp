  ...
  const Thyra::ModelEvaluatorBase::OutArgs<Scalar> &outArgs) const
{
  ...
  if ( nonnull(f_out) ||  nonnull(W_prec_out) ) { //nonnull(f_out) ||  nonnull(W_prec_out)
    // ****************
    ...
    if (nonnull(f_out)) {
      ...
      f = Thyra::get_Epetra_Vector(*f_owned_map_,f_out);//f_out?
      //f->Print(std::cout);
    }
    ...
    if (nonnull(f_out)) {//nonnull(f_out)
      Teuchos::TimeMonitor ResFillTimer(*ts_time_resfill);  
      for(int blk = 0; blk < mesh_->get_num_elem_blks(); blk++){//blk
...
#pragma omp target teams distribute parallel for device(dev) schedule(static,1) num_teams(num_elem/1024) nowait
		...
		for(int gp=0; gp < 4; gp++) {//gp
		  ...
		  const int nid = meshc[elem*n_nodes_per_elem+i];
		  
		  const int row = meshn[nid];
		  
		  const double jacwt = B.weight[0]*jac;
		  double val = 0.;
		  val = jacwt*(
		  ...
		  //f_fe_p->SumIntoGlobalValues ((int)1, &row, &val);
...
#pragma omp parallel for 
	  for (int ne=0; ne < num_elem; ne++) {//for ne
	    ...
	    for(int gp=0; gp < basis[0].ngp; gp++) {//gp
		...
		int nid = mesh_->get_node_id(blk, elem, i);
...
//   RCP< Epetra_Vector> u_old = Teuchos::rcp(new Epetra_Vector(*x_overlap_map_));
//   u_old->Import(*u_old_, *importer_, Insert);
//   alen =u_old->MyLength ();
